...
(() => {
  ...
  function open(id){
    ...
    document.querySelector('#view-dispense input[placeholder="Optional"]').value='';
    document.querySelector('#view-dispense input[placeholder="Txn ref"]').value='';
    ...
    document.querySelector('.rx-card-header .badge').textContent=selected.status==='Partial'?'Partial Dispensing':'Pending Dispensing';
  ...
  async function dispense(){
    ...
    try {
      ...
      const result=await DemoStore.transaction('dispense',{id:selected.id,expectedDispensed:selected.items.map(m=>m.dispensed||0),notes:document.querySelector('#view-dispense input[placeholder="Optional"]').value,lines:lines(),method,reference,verified:document.querySelectorAll('[data-verify]').length>0&&Array.from(document.querySelectorAll('[data-verify]')).every(e=>e.checked)});
      invoice=result.invoice;Object.assign(selected,result.prescription);data.invoices[invoice.id]=invoice;
  ...
  function success(){
    ...
    done.querySelector('div[style*="font-size:20px"]').textContent=selected.status==='Partial'?'Prescription Partially Dispensed':'Prescription Dispensed Successfully';
    done.querySelector('div[style*="font-size:13px"]').innerHTML=`Invoice <strong>${esc(invoice.id)}</strong> generated for <strong>${esc(invoice.patientName)}</strong><br>Prescription ${esc(invoice.rx)} · ${invoice.lines.length} items dispensed · ${money(invoice.total)} paid via ${esc(invoice.method)}${selected.status==='Partial'?'<br>Remaining medicines are available from the queue.':''}`;
